...
scrape_rates <- function(year, column_of_interest) {
  ...
  # load in html
  url <- paste('https://www.treasury.gov/resource-center/data-chart-center/interest-rates/Pages/TextView.aspx?data=yieldYear&year=', year, sep='')
  ...
  avg_monthly_rates <- aggregate(select(rates_data, column_of_interest), list(rates_data$Month), mean)
